    ...
    for C in [1, 0.1]:
        for penalty in ['l2']:
            for max_iter in [100, 500]:
                print(
                    f"Start of modeling with parameters: Warm start: {warm_start}, C value: {C}, Penalty: {penalty}, Solver: liblinear, Max iter: {max_iter};")

                model = LogisticRegression(warm_start=warm_start, C=C, penalty=penalty, solver='liblinear',
